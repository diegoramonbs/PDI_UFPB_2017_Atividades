...
def threshold(src, value):
    ...
    for row in range(0, src.size[0]):
        for col in range(0, src.size[1]):
            ...
            y = 255 if pixel[0] > value else pixel[0]
            i = pixel[1]
            q = pixel[2]
...
def filter1(src):
    ...
    kernel_x = [[0, -1, 0], [-1, 5, -1], [0, -1, 0]]
    #kernel_y = [[0, 1, 0], [1, -4, 1], [0, 1, 0]]
    kernel_size = 3
    kernel_middle = kernel_size/2
...
if __name__ == '__main__':
    ...
    parser.add_argument('--median',
        help='Median filter',
        type=int)
    parser.add_argument('--filter1', help='Aplication Filter1', action='store_true')
    parser.add_argument('--filter2', help='Aplication Filter2', action='store_true')
    ...
    if args.filter1:
        try:
            ...
            print("IOError, checks whether the image exists on the disk " \
                "or if it has the necessary permission to write to disk.")
